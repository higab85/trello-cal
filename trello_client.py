...
class Trello_Client(object):
    ...
    def is_logged_in(self):
        try:
            ...
            logging.warning("Not logged in: %s" % e)
        return True
...
# TEST ME
    def _log_card(self, card_id, card_hash, datetime_added, session_id):
    ...
    def log_card(self, card, session):
        self._log_card(card.id, self.card_hash(card), datetime.now(), session)

# TEST ME
    def _is_new_card(self, card_id, card_name):
        try:
    ...
    def is_new_card(self, card):
        return self._is_new_card(card.id, card.name)
...
# TEST ME
    def _find_board_id(self, board_name, boards):
    ...
    def find_board_id(self, board_name):
        ...
        return self._find_board_id(board_name, boards)
